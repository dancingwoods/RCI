...
#######################################################################
############### Functions to add to a database ########################
#######################################################################
...
LoGMasks <- function(image, scale, ksize=15, sparse=T){
	...
	if(!sparse){
		...
		ids <- unique(as.vector(masks))
		ids <- ids[which(ids>0)]
		masklist <- sapply(ids, function(id, img){return(which(img==id))}, img=masks)
		ret <- list("masks"=masklist, "dims"=dim(masks))
...
#' @export
#-
AddMasks <- function(db, calexp, method, channel=2, scales=LogSeq(0,5,50)){
	...
	if(method=="LoG"){
		...
		masks <- as.data.frame(unique(masks), stringsAsFactors=F)
		names(masks) <- c("mask")
		#return(masks)
		dbBeginTransaction(db$db)
		dbGetPreparedQuery(db$db, "INSERT INTO masks (mask) VALUES (?)", bind.data=masks)
		dbCommit(db$db)
	}
}
